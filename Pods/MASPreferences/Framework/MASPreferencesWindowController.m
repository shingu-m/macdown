...
- (void)windowDidLoad
{
    // macOS 11+: Force toolbar into its own dedicated row so all items are visible.
    // Without this, macOS Tahoe places toolbar items inside the unified title bar
    // area, reducing available width and causing items to overflow into ">>".
...
- (void)setSelectedViewController:(NSViewController <MASPreferencesViewController> *)controller
{
    ...
    // NOTE: In macOS 26 (Tahoe) and later, frameRectForContentRect: no longer
    // accounts for the toolbar height, causing the window to be sized too small
    // (bottom of content clipped, toolbar overlaps content). Use contentLayoutRect
    // to measure the actual chrome height (title bar + toolbar) instead.
    [self.window layoutIfNeeded];
    NSRect oldFrame = [self.window frame];
    CGFloat chromeHeight = NSHeight(oldFrame) - NSHeight(self.window.contentLayoutRect);
    NSRect newFrame = NSMakeRect(
        NSMinX(oldFrame),
        NSMaxY(oldFrame) - NSHeight(oldViewRect) - chromeHeight,
        NSWidth(oldViewRect),
        NSHeight(oldViewRect) + chromeHeight
    );
    newFrame = [self.window constrainFrameRect:newFrame
                                      toScreen:self.window.screen ?: [NSScreen mainScreen]];
    ...
    // macOS Tahoe: setContentMaxSize: alone may not prevent user from dragging
    // the window edge. Explicitly toggle NSWindowStyleMaskResizable to match
    // whether the selected pane actually supports resizing.
    NSWindowStyleMask mask = self.window.styleMask;
    if (sizableWidth || sizableHeight) {
